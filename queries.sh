...
echo "$($PSQL "SELECT DISTINCT name FROM teams INNER JOIN games ON winner_id=team_id OR opponent_id=team_id WHERE round='Eighth-Final' AND year=2014 ORDER BY name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT name FROM teams INNER JOIN games ON winner_id=team_id ORDER BY name")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT  games.year, name FROM teams INNER JOIN games ON winner_id=team_id WHERE round='Final' ORDER BY year")"
